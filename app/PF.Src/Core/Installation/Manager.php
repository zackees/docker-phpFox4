...
class Manager
{
    ...
    public function runInstallForModule($param)
    {
        ...
        if (!defined('PHPFOX_INSTALLER')) {
            $sModuleId = db()->select('module_id')
                ->from(':module')
                ->where([
                    'product_id' => $aParams['data']['product_id']
                ])->executeField(false);
            Phpfox::getLib('assets')->putAssetFilesForParentFolder(PHPFOX_DIR_MODULE . $sModuleId . PHPFOX_DS);
        }
...
//            unlink($filename);
    ...
    public function callRunInstallForApp(
        ...
        if (!defined('PHPFOX_INSTALLER')) {
            Phpfox::getLib('assets')->putAssetFilesForParentFolder($base);
        }
