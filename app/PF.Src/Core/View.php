...
class View
{
    ...
    public function __construct()
    {
        ...
        $this->_env = new View\Environment($this->_loader, [
            'cache'      => ((Phpfox::isTechie() || defined('PHPFOX_NO_TEMPLATE_CACHE')) ? false : self::$cache_path),
            'autoescape' => false,
        ]);
    ...
    public function getContent($force = false)
    {
        ...
        if (!$isPreviewLayout) {
            ...
            if($pageId == 'core_index-visitor') {
                $hasWelcome = $function->checkBlock([7, 2, 4], 'user.welcome');
                $hasRegister = $function->checkBlock([7, 2, 4], 'user.register');
                if($hasWelcome && !$hasRegister) {
                    $params['main_class'] .= ' welcome-only';
                }
                elseif(!$hasWelcome && $hasRegister) {
                    $params['main_class'] .= ' login-only';
                }
            }
        }
