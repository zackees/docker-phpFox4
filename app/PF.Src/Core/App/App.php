...
abstract class App
{
    ...
    private function initData()
    {
        ...
        if (!isset($this->icon) || empty($this->icon)) {
            if (cached_file_exists($this->path . 'icon.png')) {
     ...
     *
     * @param $database Database\Table | string
     */
    ...
    {
    	(($sPlugin = \Phpfox_Plugin::get('on_before_uninstall_app')) ? eval($sPlugin) : false);
        ...
        if (isset($this->alias) && !empty($this->alias)) {
            ...
            db()->delete(':cron', ['module_id' => $this->alias]);
        }
        db()->delete(':apps', 'apps_id="' . $this->id . '"');

        $this->clearAppsCache();

        (($sPlugin = \Phpfox_Plugin::get('on_after_uninstall_app')) ? eval($sPlugin) : false);
    }
    ...
    public function processInstall()
    {
        ...
        Phpfox::getService('user.group.setting.process')->importFromApp($this);
    ...
    {
        $iCurrent = Phpfox::getVersion();
        $iStart = $this->start_support_version;
        $iEnd = $this->end_support_version;
        if (($iStart <= $iCurrent) && ($iCurrent <= $iEnd || $iEnd == '')) {
            return true;
        } else {
            return false;
        }
    ...
    public function isActive()
    {
        ...
        return in_array($this->id, \Core\App::$activeApps);
    }
