...
class Functions
{
        ...
        try {
            ...
            switch ($this->_method) {
                ...
                case 'content':
                    ...
                    if (\Phpfox::isTechie() && !\Phpfox::isAdminPanel() && !PHPFOX_IS_AJAX) {
    ...
    public function checkBlock($locations, $componentName) {
        if(is_array($locations)) {
            foreach ($locations as $location) {
                $blocks = \Phpfox_Module::instance()->getModuleBlocks($location);
                if (in_array($componentName, array_column($blocks, 'component'))) {
                    return true;
                }
            }
        }
        else {
            $blocks = \Phpfox_Module::instance()->getModuleBlocks($locations);
            return in_array($componentName, array_column($blocks, 'component'));
        }
        return false;
    }
    ...
    private function _loadBlocks($location, $checkContent = false)
    {
        ...
        if($checkContent){
            $sReplaceText = str_replace(["\r","\n", "\t", "\0", "\x0B"],'', self::$blocks[$location]);
            return !!trim($sReplaceText);
        }
