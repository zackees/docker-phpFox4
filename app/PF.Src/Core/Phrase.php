...
class Phrase
{
     ...
     * @return string
     */
    public function get($sVarName = '', $aParam = [], $sLanguageId = '', $bSkipDebugPhrase = false)
    {
        ...
        if ($this->isPhrase($sVarName, false)) {
            return $this->processPhrase($sVarName, $aParam, $sLanguageId, $bSkipDebugPhrase);
        }
        ...
        if ($this->isPhrase($hash, false)) {
            return $this->processPhrase($hash, $aParam, $sLanguageId, $bSkipDebugPhrase);
        } else {
    ...
    private function processPhrase($hash, $aParam, $sLanguageId, $bSkipDebugPhrase = false)
    {
        $sVarName = isset($this->_aAllPhrases[$sLanguageId][$hash]) ? $this->_aAllPhrases[$sLanguageId][$hash] : $hash;
        $sVarName = html_entity_decode($sVarName, ENT_QUOTES, 'UTF-8');
        ...
        if ($this->enableDebugPhrase && !$bSkipDebugPhrase) {
            $sVarName = '[' . $sVarName . ']';
        }
    ...
    public function addPhrase($sVarName, $sValue = '', $bShouldClearCache = true)
    {
        ...
        foreach ($aLanguages as $aLanguage) {
            ...
            $aShouldDeleteLegacyPhrase = [];
            foreach ($aAllPhrases as $sKey => $aPhrase) {
                if (is_array($aPhrase)) {
                    $sNewText = (isset($aPhrase[$aLanguage['language_id']])) ? $aPhrase[$aLanguage['language_id']] : $aPhrase['en'];
                    if (isset($aPhrase['legacy_phrase']) && !empty($aCheckPhrases[$aPhrase['legacy_phrase']])) {
                        if ($aCheckPhrases[$aPhrase['legacy_phrase']]['text'] != $aCheckPhrases[$aPhrase['legacy_phrase']]['text_default']) {
                            $sNewText = $aCheckPhrases[$aPhrase['legacy_phrase']]['text'];
                        }
                        $aShouldDeleteLegacyPhrase[] = $aCheckPhrases[$aPhrase['legacy_phrase']]['phrase_id'];
                    }
                } else {
                    $sNewText = $aPhrase;
                }
            ...
            // @since 4.8.2
            while (($aValues = array_splice($aShouldDeleteLegacyPhrase, 0, 100)) && count($aValues) > 0) {
                $idList = implode(', ', array_map(function ($v) {
                    return intval($v);
                }, $aValues));
            ...
            while (($aValues = array_splice($aShouldInsertPhrasePool, 0, 200)) && count($aValues) > 0) {
                ...
                //Should clear cache if add new phrase
