...
//        }
        ...
        if ($bIsUpdate) {
            ...
            $bUpdateTitleUrl = isset($aVals['old_url']) && ($aVals['old_url'] != $sNewTitle);
            if ($bUpdateTitleUrl) {
            ...
            if ($bUpdateTitleUrl) {
                $componentUpdate = array_merge($componentUpdate, [
                    'component' => $aVals['title_url'],
                    'm_connection' => 'page.' . $aVals['title_url'],
                ]);
                db()->update(':block', [
                    'm_connection' => 'page.' . $aVals['title_url'],
                ], [
                    'm_connection' => 'page.' . $aVals['old_url'],
                ]);
            }

            if (!empty($componentUpdate)) {
                db()->update(':component', $componentUpdate, [
                    'module_id' => 'page',
                    'component' => $aVals['old_url'],
                    'm_connection' => 'page.' . $aVals['old_url'],
                ]);
            }
            ...
            if ($iId = $this->database()->process($aSql, $aVals)->insert($this->_sTable)) {
                db()->insert(':component', [
                    'component' => $aVals['title_url'],
                    'm_connection' => 'page.' . $aVals['title_url'],
                    'module_id' => 'page',
	...
	public function delete($iId)
	{
		...
		if (in_array((int)$aPage['page_id'], [1, 2])) {
		    return Phpfox_Error::set(_p('not_allowed_to_delete_this_page'));
        }
		if (isset($aPage['page_id'])) {
			...
			$this->database()->delete(':component', [
			   'module_id' => 'page',
               'component' => $aPage['title_url'],
            ]);
			$this->database()->delete(':block', [
			   'm_connection' => 'page.' . $aPage['title_url'],
            ]);
