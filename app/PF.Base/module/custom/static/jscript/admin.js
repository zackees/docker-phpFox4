      ...
      $('.js_option_holder').each(function () {
        ...
        $(this).find('input').each(function () {
          if ($Core.custom.aOptions !== null) {
            ...
            if (aMatches && isset(aMatches[1]) && isset($Core.custom.aOptions['option_' + (iCnt - 1) + '_' + aMatches[1]])) {
...
$Behavior.custom_admin_init = function () {
    $('.js_drop_down').off().on('click touchstart', function () {
        ...
        $('body').prepend('<div id="js_cache_menu" style="position:absolute; left:' + eleOffset.left + 'px; top:' + (eleOffset.top + 15) + 'px; z-index:100;"><span class="dropContentCancel"></span>' + $('#js_menu_drop_down').html() + '</div>');
        ...
        $('.dropContentCancel').on('click', function(e) {
          e.preventDefault();
          $('.dropContentCancel').hide();
          $('.dropContent').hide();
        ...
        $('.dropContent').on('mouseout', function () {
            $('.dropContentCancel').hide();
            $('.dropContent').hide();
            $('.sJsDropMenu').removeClass('is_already_open');
        });
    ...
    $('#var_type').off('change').on('change', function () {
    ...
    $('.js_delete_current_option').off().click(function () {
      ...
      $Core.jsConfirm({message: oTranslations['are_you_sure_you_want_to_delete_this_custom_option']}, function () {
        let deleteParams = $.getParams(obj.href);
        if (typeof deleteParams === "object" && deleteParams['id'] && $('#js_current_value_' + deleteParams['id']).length) {
          $('#js_current_value_' + deleteParams['id']).remove();
        }
      });
