            ...
            <div id="js_attachment_id_content_{$aAttachment.attachment_id}" class="item-outer">
                ...
                <div class="attachment-body">
                    ...
                    <div class="attachment-row-title">
                        ...
                        {else}
                        <a href="{url link='attachment.download' url=''$aAttachment.url''}" class="attachment_row_link no_ajax_link item-title">{$aAttachment.file_name}</a> <span class="item-info">{$aAttachment.file_size|filesize} . <span id="attachment_counter_{$aAttachment.attachment_id}"><span class="js_number">{$aAttachment.counter}</span> <span class="js_text">{if (int)$aAttachment.counter == 1}{_p var='view'}{else}{_p var='views'}{/if}</span></span></span>
                        {/if}
                    </div>
                    <div class="attachment-row-actions">
                        ...
                        {if (Phpfox::getUserParam('attachment.delete_own_attachment') && $aAttachment.user_id == Phpfox::getUserId()) || Phpfox::getUserParam('attachment.delete_user_attachment')}
                            <a role="button" onclick="$Core.jsConfirm({left_curly}message:'{_p var='are_you_sure_you_want_to_delete_this_attachment_file' phpfox_squote=true}'{right_curly},function(){left_curly}  $.ajaxCall('attachment.delete', 'id={$aAttachment.attachment_id}&editor_id={$sEditorId}');{right_curly},function(){left_curly}{right_curly}); return false;" class="js_hover_title">
