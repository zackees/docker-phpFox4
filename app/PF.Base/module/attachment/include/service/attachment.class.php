...
class Attachment_Service_Attachment extends Phpfox_Service
{
     ...
     */
    public function getItem($attachmentId)
    {
        ...
        if (!empty($attachment['attachment_id'])) {
            $attachment['url'] = sprintf($attachment['destination'], '');

            if ($attachment['is_video']) {
                $attachment['video_image_destination'] = substr_replace($attachment['destination'], '%s.jpg', -4);
            }

            $attachment['inline'] = [
                'name' => $attachment['description'] ? "$('#js_description{$attachment['attachment_id']}').html()" : $attachment['file_name'],
                'path' => $attachment['is_image'] ?
                ...
                'url'  => $attachment['is_image'] ? Phpfox::getParam('core.url_attachment') . sprintf($attachment['destination'], '') : ''
            ];
        }
     ...
     * @return array
     */
    public function get($aConds, $sSort = 'attachment.time_stamp DESC', $bCount = true, $iLimit = null)
    {
        $iCnt = 0;
        if ($bCount) {
        ...
        $aItems = $this->database()->select('attachment.*,' . Phpfox::getUserField('u'))
            ...
            ->where($aConds)
            ->order($sSort)
            ->limit($iPage, !empty($iLimit) ? $iLimit : Phpfox::getParam('attachment.attachment_item_limit', 12), $iCnt, false, false)
