...
{	
    ...
    {
        if (Phpfox::getService('language.process')->setDefault($this->get('id'), $this->get('active'))) {
            Phpfox::addMessage(_p('default_language_package_reset'));
            $this->reload();
        }
        $this->error(_p('An error occurred'));
    }

    public function updateLanguageActivity()
    {
        if (Phpfox::getService('language.process')->updateActivity($this->get('id'), $this->get('active'))) {
            $this->reload();
        }
    }
}