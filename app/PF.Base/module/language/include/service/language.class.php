...
class Language_Service_Language extends Phpfox_Service 
{
	...
	public function getForAdminCp($aConds = [])
	{
		...
		foreach ($aRows as $iKey => $aRow)
		{
            ...
            if (isset($aRow['store_id']) && $aRow['store_id']) {
                $store = Phpfox::getService('admincp.apps')->getAppInformation('', $aRow['store_id'], true);
    ...
    public function getDefaultLanguage() {
        $sCacheId = $this->cache()->set('core_default_language');
        $this->cache()->group('locale', $sCacheId);
        if (false === ($sLangId = $this->cache()->get($sCacheId))) {
            $sLangId = $this->database()->select('language_id')
                ->from(Phpfox::getT('language'))
                ->where('is_default=1')
                ->execute('getSlaveField');
            $this->cache()->save($sCacheId, $sLangId);
        }
