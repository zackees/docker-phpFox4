...
class Language_Service_Phrase_Process extends Phpfox_Service 
{
     ...
     * @param null $sNewVarName
     * @return bool
     */
	public function updateVarName($sLanguage, $sVarName, $sValue, $bOverWrite = false, $sNewVarName = null)
	{
        //Support legacy phrase. End support from 4.7.0
        ...
        if (!empty($sNewVarName)) {
            $aSql['var_name'] = $sNewVarName;
        }
        if ($bOverWrite) {
     ...
     * @param bool   $bIsOverride
     *
     * @return bool|string
     * @throws Exception
     *
     */
	public function installFromFolder($sPack, $sDir, $iPage = 0, $iLimit = 5, $aLanguage = [], $bIsOverride = false)
	{
        ...
        while ($sFile = readdir($hDir)) {
            ...
            if (preg_match('/^module-(.*?)\.xml$/i', $sFile, $aMatches)) {
                if (Phpfox::isModule($aMatches[1])) {
                    ...
                    if (!empty($aPhrases['phrase'])) {
                        $aRows = (isset($aPhrases['phrase'][1]) ? $aPhrases['phrase'] : [$aPhrases['phrase']]);
                        foreach ($aRows as $key => $aPhrase) {
                            $bIsAddNew = false;
                            if (!empty($aLanguage)) {
                                if (isset($aLanguage[$aPhrase['var_name']])) {
                                    if ($bIsOverride) {
                                        if (is_array($aLanguage[$aPhrase['var_name']])) {
                                            if ($aPhrase['var_name'] == $aLanguage[$aPhrase['var_name']]['var_name']) {
                                                $iPhraseId = $aLanguage[$aPhrase['var_name']]['phrase_id'];
                                            } else {
                                                $iPhraseId = null;
                                            }
                                        } else {
                                            $iPhraseId = $aLanguage[$aPhrase['var_name']];
                                        }
