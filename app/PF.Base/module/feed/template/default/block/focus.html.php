...
<div class="activity_feed_content_text{if isset($aFeed.comment_type_id) && $aFeed.comment_type_id == 'poll'} js_parent_module_feed_{$aFeed.comment_type_id}{/if}{if !empty($aFeed.custom_class)} {$aFeed.custom_class}{/if}{if !empty($aFeed.status_background)} p-statusbg-feed" style="background-image: url('{$aFeed.status_background}');"{else}"{/if}>
        ...
        {if isset($aFeed.feed_status) && (!empty($aFeed.feed_status) || $aFeed.feed_status == '0')}
            <div class="activity_feed_content_status">{if !empty($aFeed.is_feed_status_handled)}{$aFeed.feed_status}{else}{if strpos($aFeed.feed_status, '<br />') >= 200}{$aFeed.feed_status|feed_strip|shorten:200:'feed.view_more':true|split:55}{else}{$aFeed.feed_status|feed_strip|split:55|shorten:200:'feed.view_more':true}{/if}{/if}
            </div>
        {/if}
    ...
    {if Phpfox::getParam('feed.enable_check_in') && Phpfox::getParam('core.google_api_key') != '' && !empty($aFeed.location_name)
        && isset($aFeed.location_latlng) && isset($aFeed.location_latlng.latitude) && $aFeed.type_id != 'photo' && $aFeed.type_id != 'v'
        && $aFeed.type_id != 'link' && empty($aFeed.status_background) && (empty($aFeed.parent_feed_id) || !empty($aFeed.feed_reference))}
