...
class Ban_Service_Ban extends Phpfox_Service
{
     ...
     */
    public function getFilters($sType, $iPage = 0, $iLimit = 10, &$iCnt = 0, $sQuery = '')
    {
        $sCacheId = $this->cache()->set('ban_type_' . $sType);
        if (false === ($aFilters = $this->cache()->get($sCacheId)) || $iPage != 0) {
            $sQuery = $this->preParse()->reversePrepare($sQuery);
            $aConditions = [
                'b.type_id' => $this->database()->escape($sType),
                " AND (b.find_value LIKE '%{$sQuery}%' OR b.replacement LIKE '%{$sQuery}%')"
            ];
            if($iPage) {
            ...
            db()->select('b.*, ' . Phpfox::getUserField())
                ->from($this->_sTable, 'b')
                ->leftJoin(Phpfox::getT('user'), 'u', 'u.user_id = b.user_id')
                ->where($aConditions);

            if($iPage && $iLimit) {
                db()->limit($iPage, $iLimit, $iCnt);
            }
            $aFilters = db()->execute('getSlaveRows');
     ...
     * @return bool
     */
    public function check($sType, $sValue, $bGetReason = false, $sSubType = '')
    {
        if ($sSubType == 'phone_number' && !Phpfox::getParam('core.enable_register_with_phone_number')) {
            return true;
        }
        ...
        if (is_array($aFilters) && count($aFilters)) {
            foreach ($aFilters as $sFilter => $mValue) {
                ...
                if ($sSubType == 'phone_number') {
                    $sFilter = preg_replace('/([()\-.\\\s]+)/', '', $sFilter);
                    $sValue = preg_replace('/\s/', '', $sValue);
                    $sFilter = preg_replace('/^\+/', '\+', $sFilter);
                }
    ...
    public function checkAutomaticBan($sValue)
    {
        ...
        foreach ($aFilters as $iKey => $aFilter) {
            ...
            if ($bBan) {
                ...
                if (strpos($aFilter['reason'], '{phrase var=') > -1) {
                ...
                } elseif (defined('PHPFOX_IS_AJAX') && PHPFOX_IS_AJAX) {
