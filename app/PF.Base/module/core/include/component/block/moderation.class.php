...
class Core_Component_Block_Moderation extends Phpfox_Component
{
	...
	public function process()
	{
        ...
        if (!empty($aParams['menu'])) {
            foreach ($aParams['menu'] as $key => $aMenuParam) {
                if (!empty($aMenuParam['message'])) {
                    $aParams['menu'][$key]['message'] = Phpfox::getLib('parse.output')->htmlspecialchars($aMenuParam['message']);
                }
            }
        }
