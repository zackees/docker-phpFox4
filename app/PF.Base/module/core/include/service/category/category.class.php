...
class Core_Service_Category_Category extends Phpfox_Service
{
	...
	public function get()
	{
		...
		{
			if (false === ($sOutput = $this->cache()->get($sCacheId)))
			{				
		...
		{
			if (false === ($this->_sOutput = $this->cache()->get($sCacheId)))
			{				
				$this->_get(0, 1);
	...
	{		
		...
		if (false === ($aBreadcrumb = $this->cache()->get($sCacheId)))
		{		
			$sCategories = $this->getParentCategories($sCategory);
	...
	public function getAllCategories($sCategory)
	{
		...
		if (false === ($sCategories = $this->cache()->get($sCacheId)))
		{
	...
	private function _getParentsUrl($iParentId, $bPassName = false)
	{
		...
		if ($iCnt === 1)
		{
			...
			if (false !== ($sParents = $this->cache()->get($sCacheId)))
			{
				return $sParents;
			}
		}
