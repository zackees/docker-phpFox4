...
class Core_Service_Currency_Process extends Phpfox_Service 
{
	...
	public function updateDefault($iId, $sType)
	{
		...
		$this->cache()->removeGroup('currency');
	}
	...
	{
        $aCurrency = Phpfox::getService('core.currency')->getForEdit($sId);
        if (empty($aCurrency['currency_id'])) {
            return Phpfox_Error::set(_p('currency_not_found'));
        }
        if ($aCurrency['is_default']) {
            return Phpfox_Error::set(_p('cannot_delete_default_currency'));
        }
