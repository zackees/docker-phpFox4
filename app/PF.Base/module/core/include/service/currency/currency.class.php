...
class Core_Service_Currency_Currency extends Phpfox_Service 
{
	...
	{
        if(!$sCurrencyId || !isset($this->_aCurrencies[$sCurrencyId])) {
    ...
    public function getFieldPrice($sCurrencyName = 'currency_id', $sPriceName = 'price', $bCloseWarning = false)
    {
        ...
        $sReturn .= '<select class="form-control' . ($bCloseWarning ? ' close_warning' : '') . '" id="' . $sCurrencyName . '" name="val[' . $sCurrencyName . ']">';
        ...
        $sReturn .= '<input class="form-control' . ($bCloseWarning ? ' close_warning' : '') . '" type="text" name="val[' . $sPriceName . ']" value="' . $sPriceDefault . '" id="' . $sPriceName . '" size="10" maxlength="100" onfocus="this.select();" />';
        $sReturn .= '</div>';
        return $sReturn;
    }
