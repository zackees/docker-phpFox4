 ...
 * @author  		phpFox LLC
 * @package 		Phpfox
 * @version 		$Id: activity.html.php 3326 2011-10-20 09:12:45Z phpFox LLC $
 */
		...
		function doGiftPoints()
        {
            ...
            if ($.isNumeric(iAmount) == false || (Math.floor(iAmount) != iAmount)) {
                ...
                $.ajaxCall('core.doGiftPoints', 'user_id={/literal}{$iTrgUserId}{literal}&amount=' + iAmount);
                 $('#div_show_gift_points .js_gift_point_btn_disable_check').prop('disabled', true);
            }
        }
        {/literal}
	</script>
	<div class="extra_info">
		{_p var='you_are_about_to_gift_activity_points' full_name=$aUser.full_name current=$iCurrentAvailable|number_format}
	</div>
	{if $iCurrentAvailable > 1}
		<div class="p_top_8">
			{if $iCurrentAvailable == 1}
				...
				<div class="p_top_8">
					<input type="button" value="{_p var='yes'}" class="button btn-primary js_gift_point_btn_disable_check" onclick="doGiftPoints();" />
                </div>
			{else}
				{_p var='how_many_points_do_you_want_to_gift_away'}
				<div class="p_top_8">
					<input type="text" id="txt_amount_points" value="" class="form-control" /> <input type="button" value="{_p var='gift_points'}" class="button btn-primary m_top_5 js_gift_point_btn_disable_check" onclick="doGiftPoints();" />
                </div>
                <div class="p_top_8">
