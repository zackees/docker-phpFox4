...
{if $bIsListType}
    <div class="{if $sPrivacyFormType == 'mini'}privacy_setting_mini{else}privacy_setting{/if} privacy_setting_div privacy-list-type">
        ...
        <ul class="{if !empty($bSelectInline)}privacy_setting_inline{/if}">
            {foreach from=$aPrivacyControls name=privacycontrol item=aPrivacyControl}
                <li role="presentation" class="{if (isset($aPrivacyControl.is_active)) || (isset($bNoActive) && $bNoActive && $phpfox.iteration.privacycontrol == 1)}is_active_image{/if}">
                    <a {if isset($aPrivacyControl.onclick)} onclick="{$aPrivacyControl.onclick} return false;"{/if} data-toggle="privacy_item" rel="{$aPrivacyControl.value}"
                    {if !empty($bSelectInline)}data-privacy-inline="true"{/if}
                    class="{if (isset($aPrivacyControl.is_active)) || (isset($bNoActive) && $bNoActive && $phpfox.iteration.privacycontrol == 1)}is_active_image{/if} {if !empty($bSelectInline)}btn btn-icon{/if}">
                        <i class="fa fa-privacy fa-privacy-{$aPrivacyControl.value}"></i>
                        <span class="txt-label">{$aPrivacyControl.phrase}</span>
                    </a>
                </li>
            {/foreach}
        </ul>
