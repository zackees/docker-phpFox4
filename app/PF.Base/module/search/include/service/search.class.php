...
{
    public function query($sQuery, $iPage, $iTotalShow, $sView = null, &$iRootTotalPerPage = 0)
    {
        if ($sView !== null && Phpfox::isModule($sView) && Phpfox::hasCallback($sView, 'globalUnionSearch')) {
            $result = Phpfox::callback($sView . '.globalUnionSearch', $this->preParse()->clean($sQuery));
            if ($result === false) {
                //This module don't allow search
                return [];
            }
        ...
        db()->select('item.*, ' . Phpfox::getUserField())
            ->unionFrom('item', true);

        if (!empty($aBlockedUserIds = Phpfox::getService('user.block')->get(null, true))) {
            db()->where('u.user_id NOT IN (' . implode(',', $aBlockedUserIds) . ')');
        }

        $aRows = db()->join(Phpfox::getT('user'), 'u', 'u.user_id = item.item_user_id')
            ->limit($iPage, $iTotalShow)
            ->order('item_time_stamp DESC')
            ->execute('getSlaveRows');

        $iRootTotalPerPage = count($aRows);
        $aResults = [];
    ...
    public function getSearchWords($iPage, $iLimit)
    {
        $aWords = $this->database()->select('*')
        ...
        return [$this->database()->getCount(), $aWords];
    }
