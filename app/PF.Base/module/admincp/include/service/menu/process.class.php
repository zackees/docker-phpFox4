...
class Admincp_Service_Menu_Process extends Phpfox_Service 
{
	...
	public function importFromApp($App)
    {
        ...
        $aParams = [
            'm_connection' => isset($App->menu->location) ? $App->menu->location : 'main',
            'module_id' => $App->alias,
            'product_id' => 'phpfox',
            'allow_all' => true,
        ...
        if (isset($App->menu->ordering)) {
            $aParams['ordering'] = $App->menu->ordering;
        }
	...
	public function add($aVals, $bIsUpdate = false, $bCheckDuplicate = false, $bAddPhrase = true)
	{
		...
		} elseif ($aVals['m_connection'] == 'explore' || $aVals['m_connection'] == 'main') {
			$aInsert['parent_id'] = !empty($aVals['parent_id']) ? $aVals['parent_id'] : 0;
		} elseif ($aVals['m_connection'] == 'footer') {
            $aInsert['parent_id'] = 0; //Doesn't support parent menu in footer
        }
		
		if ($bIsUpdate) {
			$this->database()->update($this->_sTable, $aInsert, 'menu_id = ' . (int) $aVals['menu_id']);
			if ($aVals['m_connection'] == 'footer') {
			    //Footer menu doesn't support children
			    $this->database()->update($this->_sTable, ['parent_id' => 0], 'parent_id = ' . (int)$aVals['menu_id']);
            }
		}
		else {
