...
class Admincp_Service_Component_Component extends Phpfox_Service
{
    ...
    public function getComponentsByController($sController = '')
    {
        $aComponents = $this->get();

        if (!empty($sController) && !in_array($sController, ['site_wide'])) {
            foreach ($aComponents as $key => $aComponent) {
                foreach ($aComponent as $itemKey => $itemValue) {
                    if (!empty($itemValue['allowed_connections'])
                        && !empty($aAllowedConnections = unserialize($itemValue['allowed_connections']))
                        && !in_array($sController, $aAllowedConnections)) {
                        unset($aComponents[$key][$itemKey]);
                    }
                }
            }
        }
