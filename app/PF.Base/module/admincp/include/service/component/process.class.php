        ...
        if (isset($App->component->block)) {
            foreach ($App->component->block as $key => $value) {
                ...
                $iCnt = db()->select('COUNT(*)')
                    ...
                    ->where([
                        'component' => $key,
                        'm_connection' => (isset($blockParams['m_connection']) ? $blockParams['m_connection'] : ''),
                        'module_id' => $App->alias,
                        'is_controller' => 0,
                    ])->executeField();
                if ($iCnt) {
                ...
                $InsertData[] = [
                    $key, //component
                    (isset($blockParams['m_connection']) ? $blockParams['m_connection'] : ''),//m_connection
                    $App->alias,//module_id
                    'phpfox',//product_id
                    0,//is_controller
                    1,//is_block
                    1,//is_active,
                    (!empty($blockParams['allowed_connections']) ? serialize($blockParams['allowed_connections']) : null),//allowed_connections
                ];
            }
        }
