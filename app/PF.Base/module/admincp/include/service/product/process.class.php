...
class Admincp_Service_Product_Process extends Phpfox_Service
{
	...
	public function delete($sProduct)
	{
        ...
        $aProduct = $this->database()->select('p.*, m.module_id')
            ->from($this->_sTable, 'p')
            ->join(':module', 'm', 'm.product_id = p.product_id')
            ->where('p.product_id = \'' . $this->database()->escape($sProduct) . '\'')
            ->execute('getSlaveRow');

        if (!isset($aProduct['product_id']) || !isset($aProduct['module_id'])) {
        ...
        db()->delete(':cron', ['module_id' => $aProduct['module_id']]);
        $this->cache()->remove();
