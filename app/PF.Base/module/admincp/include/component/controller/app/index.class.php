...
class Admincp_Component_Controller_App_Index extends Phpfox_Component
{
    public function process()
    {
        ...
        if ($this->request()->get('verify')) {
            if ($appInit = \Core\Lib::appInit($sAppId)) {
        ...
        $has_upgrade = false;
        $store = Phpfox::getService('admincp.apps')->getAppInformation($App->id, isset($App->store_id) ? $App->store_id : '');
        if (isset($store['id']) && version_compare($App->version, $store['version'], '<')) {
            ...
            $store['install_url'] = isset($store['link']) ? $store['link'] : $store['url'] . '/installing?iframe-mode=' . $response->token;
            $has_upgrade = true;
        }
