...
class Admincp_Component_Controller_Index extends Phpfox_Component
{
    ...
    public function process()
    {
        ...
        $this->template()->setHeader('<script>var admincpSettings = ' . json_encode(Phpfox::getService('admincp')->getForSearch()) . ';</script>');
        ...
        if (Phpfox::isAppAlias($app) or Phpfox::isApps($app)) {
            ...
            if ($App->admincp_menu) {
                ...
                if (!$is_settings && !$is_group_settings) {
                    $sCurrentUrl = trim(str_replace($this->url()->makeUrl('admincp'), '', $this->url()->current()), '/');
                    if (preg_match('/id=' . $sAppId . '/', $sCurrentUrl) || $sCurrentUrl == $sAppAlias) {
                        $bIsAppIndex = true;
                    } else {
                        if (($pos = strpos($sCurrentUrl, '?')) !== false) {
                            $sCurrentUrl = rtrim(substr_replace($sCurrentUrl, '', $pos), '/');
                        }
                ...
                foreach ($App->admincp_menu as $key => $value) {
                    ...
                    if (($bIsAppIndex && $value == '#') || (!empty($sCurrentUrl) && $sCurrentUrl == $value)) {
        ...
        if ($bPass) {
            if (Phpfox::isModule($this->_sModule) || (Phpfox::isAppAlias($this->_sModule) && Phpfox::isAppActive($this->_sModule))) {
