    ...
    public function process()
    {
        ...
        if ($aVals = $this->request()->getArray('val')) {
            if (empty($aVals['m_connection'])) {
                ...
                if ($bIsEdit) {
                    $this->url()->send('admincp.menu', !empty($aVals['parent_id']) ? ['parent' => $aVals['parent_id']] : null, $sMessage);
                } else {
                    $this->url()->send('admincp.menu', !empty($aVals['parent_id']) ? ['parent' => $aVals['parent_id']] : null, $sMessage);
                }
            }
        ...
        $this->template()->assign(array(
                ...
                'aParents' => Phpfox::getService('admincp.menu')->get(['menu.parent_id' => 0, 'menu.is_active' => 1, 'menu.m_connection' => 'main'], false),
                ...
                'bIsEdit' => $bIsEdit,
                'bIsPage' => $bIsPage,
                'sThemeActive' => flavor()->active->id,
                'sLineficonLink' => Phpfox::getParam('core.path_actual') . 'PF.Site/flavors/material/component/linefico.html'
            )
        )
            ->setActiveMenu('admincp.appearance.menu')
            ->setBreadCrumb(_p($bIsEdit ? 'edit_menu' : 'add_new_menu'), $this->url()->makeUrl('current'), true)
            ->setTitle(_p($bIsEdit ? 'edit_menu' : 'add_new_menu'));
    }
}