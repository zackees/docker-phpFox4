...
<script type="text/javascript">
	...
	var sSearchByValue = '{$sSearchByValue}';
{literal}
...
<div id="js_user_loader" class="core-user-browser-wrapper">
...
{/if}
	<div id="js_user_search_content">
		<div class="core-user-browser-pager">
			{pager}
		</div>
		<div class="main_break"></div>
		<div class="label_flow" style="height:200px;">
		{foreach from=$aUsers name=users item=aUser}
			<div class="core-user-browser-row {if is_int($phpfox.iteration.users/2)}row1{else}row2{/if}{if $phpfox.iteration.users == 1} row_first{/if}">
				<span class="core-user-browser-checkbox"><input type="checkbox" class="checkbox" name="user[]" class="js_users_checkbox" id="js_users_checkbox_{$aUser.user_id}" value="{$aUser.user_id}" onclick="addUserToSelectList(this, '{$aUser.user_id}');" style="vertical-align:middle;" /></span>
				<span class="core-user-browser-title" id="js_user_{$aUser.user_id}">{$aUser|user:'':' onclick="return plugin_userLinkClick(this);"'}</span>
			</div>
		{foreachelse}
...
{if !$bIsAjaxSearch}	
	<div class="main_break t_right core-user-browser-btns">
