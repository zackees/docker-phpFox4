...
<div class="page_section_menu page_section_menu_header">
	...
	<ul class="nav nav-tabs nav-justified">
		...
		{if count($aGroups)}
          ...
          <a href="#{$aGroup.group_id}" class="js_custom_change_group" id="group_{$aGroup.group_id}">{$aGroup.phrase_var_name_text}</a>
        </li>
			{/foreach}
		{else}
    ...
    <div class="js_custom_groups js_custom_group_basic">
	    <div class="form-group js_core_init_selectize_form_group">
            <label for="country_iso">{if Phpfox::getParam('user.require_basic_field')}{required}{/if}{_p var='location'}:</label>
            {select_location}
		...
		<div class="form-group js_core_init_selectize_form_group">
			<label>{if Phpfox::getParam('user.require_basic_field')}{required}{/if}{_p var='date_of_birth'}:</label>
		...
		<div class="form-group">
            <label for="gender">{if Phpfox::getParam('user.require_basic_field')}{required}{/if}{_p var='gender'}:</label>
            {select_gender}
		</div>
		{/if}
						...
						<div class="edit_friend_relation">
                            ...
                            <span id="sFriendImage">
                                {if isset($aForms.with_user) &&  !empty($aForms.with_user) && $aForms.with_user.with_user_id > 0}
                                    {img user=$aForms.with_user suffix='_120_square' max_width=30 max_height=30}
                                {/if}
                            </span>
	...
	{if count($aSettings)}
		{foreach from=$aSettings item=aSetting}
			<div class="form-group {if $aSetting.var_type != 'date'} js_core_init_selectize_form_group {/if} js_custom_groups js_custom_group_{$aSetting.group_id}" style="display:none;">
				<label class="table_left">
                {if $aSetting.is_required}{required}{/if}
                {$aSetting.phrase_var_name_text}:
				</label>
