            ...
            <div class="form-group">
                {if !empty($aReasons)}
                    ...
                    {foreach from=$aReasons item=aReason name=reasons}
                        <div class="p_2 checkbox">
                            <label>
                                <input type="checkbox" name='val[reason][]' value="{$aReason.delete_id}" class="v_middle"/> {$aReason.phrase_text}
                            </label>
                        </div>
                    {/foreach}
                {/if}
                ...
                {if !Phpfox::getUserBy('fb_user_id') && !Phpfox::getUserBy('janrain_user_id')}
                    <div class="form-group">
                        ...
                        <input  class="form-control" type="password" id="password" name="val[password]" size="20" autocomplete="off" data-password-viewable="1"ld/>
                    </div>
                {/if}
                <div class="form-group">
                    <input type="submit"
                                   data-message="{_p var='are_you_absolutely_sure_this_operation_cannot_be_undone' phpfox_squote=true}"
                                   class="btn btn-danger sJsConfirm" value="{_p var='delete_my_account'}" data-left="1"/>
