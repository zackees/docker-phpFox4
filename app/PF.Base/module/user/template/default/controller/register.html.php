...
{if Phpfox::isModule('invite') && Phpfox::getService('invite')->isInviteOnly()}
    ...
    {if Phpfox::getParam('core.enable_register_with_phone_number')}
        {module name='user.phone-number-country-codes' init_onchange=1 phone_field_id='#invite_email'}
    {/if}
{else}
    {if isset($sCreateJs)}
        {$sCreateJs}
    ...
    <div class="main_break" id="js_registration_holder">
        <form method="post" class="form" action="{url link='user.register'}" id="js_form" enctype="multipart/form-data" data-step="{if !empty($bIsPosted)}2{else}1{/if}">
            {token}
            ...
            {if Phpfox::getParam('user.new_user_terms_confirmation')}
                <div id="js_register_accept" class="js_register_step1" {if !empty($bIsPosted)}style="display: none"{/if}>
                    <div class="checkbox">
                        <label>
            ...
            <div class="form-button-group register-form-button-group">
                <div class="form-group">
                    ...
                    {else}
                        <span {if empty($bIsPosted)}style="display: none"{/if}><input type="button" value="{_p var='previous_step'}" class="btn btn-default text-uppercase" id="js_registration_back_previous" onclick="$Core.registration.backPreviousForm();"/></span>
                        <input type="button" value="{if !empty($bIsPosted)}{_p var='continue'}{else}{_p var='next_step'}{/if}" class="btn btn-success text-uppercase" id="js_registration_submit" onclick="$Core.registration.submitForm();" />
                    {/if}
                </div>
