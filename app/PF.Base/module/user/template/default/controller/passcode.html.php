...
<div class="form-group">
    ...
    {if $sQRCodeUrl && $iPassValidate}
        ...
        <div>
            <div class="mb-1">
                <strong>{_p var='change_two_step_qr_code_notice'}</strong>
            </div>
            <a class="btn btn-danger sJsConfirm" href="{url link='user.passcode' change_code=1}" data-message="{_p var='are_you_sure_you_want_to_revoke_two_step_qr_code'}">{_p var='generate_new_qr_code'}</a>
        </div>
    {/if}
</div>
{if !$iPassValidate}
    {if !empty($bRequiredChangePassword)}
        <div class="error_message">{_p var='notice_user_to_change_password_first' link=$sAccountSettingLink}</div>
    {else}
        <form method="post" class="form">
            <div class="form-group">
                <label for="email">{required}{_p var='enter_your_password'}</label>
                <input class="form-control" id="password" type="password" required name="val[password]" value="" placeholder="{_p var='password'}" data-password-viewable="1"/>
