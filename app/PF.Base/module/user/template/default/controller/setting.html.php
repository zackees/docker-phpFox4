...
<div class="main_break">
	<form class="form" method="post" action="{url link='user.setting'}" id="js_form" onsubmit="{$sGetJsForm}">
		...
		<div class="form-group">
            ...
            <input autocomplete="off" class="form-control" type="text" name="val[first_name]" id="first_name" value="{value type='input' id='first_name'}" size="30" {if $iTotalFullNameChangesAllowed != 0 && $aForms.total_full_name_change >= $iTotalFullNameChangesAllowed}readonly="readonly"{/if} />
		</div>

		<div class="form-group">
            <label for="last_name" for="last_name">{required}{_p var='last_name'}:</label>
            <input autocomplete="off" class="form-control" type="text" name="val[last_name]" id="last_name" value="{value type='input' id='last_name'}" size="30" {if $iTotalFullNameChangesAllowed != 0 && $aForms.total_full_name_change >= $iTotalFullNameChangesAllowed}readonly="readonly"{/if} />
		</div>
		...
		<div class="form-group">
            ...
            {/if}
            <div><input type="hidden" name="val[old_user_name]" id="old_user_name" value="{value type='input' id='old_user_name'}" size="30" /></div>
		</div>
		{/if}
        <div class="form-group">
            ...
            {else}
                <input class="form-control" type="text" readonly id="email" value="{value type='input' id='email'}" size="30" />
                <input type="hidden" name="val[email]" value="{value type='input' id='email'}" size="30" />
            {/if}
        ...
        {if Phpfox::getParam('core.enable_register_with_phone_number')}
            <div class="form-group">
                ...
                {else}
                    <input class="form-control" type="text" readonly id="phone_number" value="{value type='input' id='phone_number'}" size="30" />
                    <input type="hidden" name="val[phone_number]" value="{value type='input' id='phone_number'}" size="30" />
                {/if}
        ...
        {if Phpfox::getUserParam('user.can_use_2step_verification')}
            ...
            <div id="js_two_step_confirm_password" style="display: none">
                <div class="form-group">
                    {if !empty($bRequiredChangePassword)}
                        <div class="error_message">{_p var='notice_user_to_change_password_first_no_link'}</div>
                    {else}
                        <label>{required}{_p var='enter_your_password'}</label>
                        <input type="password" name="val[two_step_confirm_password]" class="form-control" data-password-viewable="1"/>
