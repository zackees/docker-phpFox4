...
class User_Service_Group_Setting_Process extends Phpfox_Service
{
    ...
    public function importFromApp($App)
    {
        ...
        foreach ($aUserGroupSettings as $sVarName => $aRow) {
            ...
            $aDisallow = isset($aRow['disallow_access']) ? $aRow['disallow_access'] : [];
            ...
            $aVals = [
                ...
                'disallow_access' => !empty($aDisallow) ? serialize($aDisallow) : null
            ];
            ...
            $aPhrases["user_setting_{$sAlias}_{$sVarName}"] = [
                'en' => $sPhraseValue,
                'legacy_phrase' => "user_setting_{$sVarName}"
            ];

            if ($aExists) {
                if ($aExists['type_id'] != $aVals['type_id']) {
                    $this->deleteOldSetting($aVals['name'], $aVals['module_id']);
                } elseif (isset($aVals['overwrite']) and $aVals['overwrite']) {
                    $this->deleteOldSetting($aVals['name'], $aVals['module_id']);
                } elseif ($aExists['product_id'] != $aVals['product_id']) {
                    $this->database()->update($this->_sTable, ['product_id' => $aVals['product_id']], 'setting_id=' . $aExists['setting_id']);
                    continue;
                } elseif ($aExists['disallow_access'] != $aVals['disallow_access']) {
                    $this->database()->update($this->_sTable, ['disallow_access' => $aVals['disallow_access']], 'setting_id=' . $aExists['setting_id']);
                    continue;
                } else {
                    continue;
                }
            ...
            $iSettingId = $this->database()->insert($this->_sTable, array(
                    ...
                    'option_values' => $sOptions,
                    'disallow_access' => $aVals['disallow_access']
                )
            );
    ...
    public function addSetting($aVals)
    {
        ...
        if ($bIsExists) {
            if (isset($aVals['type_id']) && ($bIsExists['type_id'] != $aVals['type_id'])) {
        ...
        Phpfox::getService('language.phrase.process')->add(array(
                'var_name' => 'user_setting_' . $aModules[0] . '_' . $aVals['name'],
                'text' => $aVals['text']
            )
        );
    ...
    public function updateSetting($aVals)
    {
        ...
        $phraseVarName = 'user_setting_' . $aModules[0] . '_' . $aVals['name'];
        $oldPhraseVarName = 'user_setting_' . $aVals['name'];
        if (Core\Lib::phrase()->isPhrase($oldPhraseVarName)) {
            foreach ($aVals['text'] as $sLang => $sValue) {
                Phpfox::getService('language.phrase.process')->updateVarName($sLang,
                    $phraseVarName, $sValue, false, $phraseVarName);
            }
        } elseif (Core\Lib::phrase()->isPhrase($phraseVarName)) {
            foreach ($aVals['text'] as $sLang => $sValue) {
                Phpfox::getService('language.phrase.process')->updateVarName($sLang,
                    $phraseVarName, $sValue);
            }
        } else {
            Phpfox::getService('language.phrase.process')->add([
                'var_name' => $phraseVarName,
                'text' => $aVals['text']
            ]);
        }
    ...
    public function update($iGroupId, $aVals)
    {
        ...
        foreach ($aVals['value_actual'] as $iId => $sValue) {
            ...
            switch ($aSettings[$iId]) {
                ...
                case 'multi_checkbox':
                    if (empty($sValue)) {
                        ...
                        $sValue = array_diff($sValue, ['core_multi_checkbox_off']);
                        $sValue = serialize($sValue);
                    }
                    break;
            }
            ...
            $aSql[] = [$iGroupId, $iId, $sValue];

        }

        foreach ($aSql as $aRow) {
            $this->database()->delete(Phpfox::getT('user_setting'), ['user_group_id' => $aRow[0], 'setting_id' => $aRow[1]]);
            $this->database()->insert(Phpfox::getT('user_setting'), ['user_group_id' => $aRow[0], 'setting_id' => $aRow[1], 'value_actual' => $aRow[2]]);
        }
        ...
        // Mass callback
        Phpfox::massCallback('onUpdateUserGroupSettings', $iGroupId, $aVals);
