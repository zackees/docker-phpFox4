...
class User_Service_Group_Setting_Setting extends Phpfox_Service
{
    ...
    public function getForSearch($aSkipModules = [])
    {
        ...
        $aSettings = $this->database()
            ...
            ->where('user_group_setting.is_hidden = 0')
            ->execute('getSlaveRows');

        $oPhrase = Core\Lib::phrase();
        foreach ($aSettings as $aRow) {
            $aRows[] = [
                'module_id' => $aRow['module_id'],
                'title' => strip_tags(htmlspecialchars_decode(($oPhrase->isPhrase("user_setting_{$aRow['module_id']}_{$aRow['name']}") ? _p("user_setting_{$aRow['module_id']}_{$aRow['name']}")
                    : ($oPhrase->isPhrase("user_setting_{$aRow['name']}") ? _p("user_setting_{$aRow['name']}") : $aRow['name'])))),
    ...
    public function get($iGroupId, $iModuleId = null)
    {
        ...
        foreach ($aRows as $aRow) {
            ...
            $aDisallowAccess = !empty($aRow['disallow_access']) ? unserialize($aRow['disallow_access']) : [];

            if (in_array($iGroupId, $aDisallowAccess)
                || (isset($aRow['inherit_id']) && in_array($aRow['inherit_id'], $aDisallowAccess))) {
                continue;
            }

            $aParts = explode('</title><info>',
                Phpfox::isPhrase("user_setting_{$aRow['module_id']}_{$aRow['name']}")
                    ? _p("user_setting_{$aRow['module_id']}_{$aRow['name']}") : (Phpfox::isPhrase("user_setting_{$aRow['name']}") ? _p("user_setting_{$aRow['name']}") : $aRow['name']));

            $aRow = array_merge($aRow, [
                'setting_name' => str_replace("\n", "<br />", strip_tags($aParts[0])),
                'setting_info' => isset($aParts[1]) ? $aParts[1] : '',
                'user_group_id' => $sVar,
                'values' => unserialize($aRow['option_values']),
            ]);
    ...
    public function getSetting($iId)
    {
        ...
        $aRow['info'] = (Core\Lib::phrase()->isPhrase("user_setting_{$aRow['module_id']}_{$aRow['name']}") ? _p("user_setting_{$aRow['module_id']}_{$aRow['name']}") :
            (Core\Lib::phrase()->isPhrase("user_setting_{$aRow['name']}") ? _p("user_setting_{$aRow['name']}") : $aRow['name']));
    ...
    private function &_setType(&$aRow, $sVar)
    {
        ...
        switch ($aRow['type_id']) {
            ...
            case 'currency':
            case 'multi_checkbox':
                $aRow['value_actual'] = Phpfox::getLib('setting')->getActualValue($aRow['type_id'], $aRow['value_actual'], $aRow);
                break;
        }
