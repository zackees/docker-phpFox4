...
class User_Service_Block_Block extends Phpfox_Service
{
    ...
    /**
     * Improve performance: Reduce cache call in 1 request
     * Fixed bug: if no data, called both database and cache
     * @var array
     */
    private $blockedList = [];

    private function getBlockedUserIds($iUserId)
    {
        if (isset($this->blockedList[$iUserId])) {
            return $this->blockedList[$iUserId];
        }

        $cache = $this->cache()->set('user_block_both_' . $iUserId);
        $aUserIds = $this->cache()->getLocalFirst($cache, 60);
        if ($aUserIds === false) {
            $aRows = $this->database()
            ...
            $this->cache()->saveBoth($cache, $aUserIds);
            Phpfox::getLib('cache')->group('user', $cache);
        }

        /** Case in current request */
        $this->blockedList[$iUserId] = $aUserIds;
        return $aUserIds;
    }
    ...
    public function get($iUserId = null, $bBothSide = false)
    {
        ...
        if ($bBothSide) {
            // Improve performance reduce database query
            $aUserIds = $this->getBlockedUserIds($iUserId);
