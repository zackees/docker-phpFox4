...
class User_Component_Controller_Admincp_Cancellations_Add extends Phpfox_Component
{
	...
	public function process()
	{
		...
		if (($iId = $this->request()->getInt('id')))
		{
            ...
            $aEdit = reset($aEdit);
            $bIsEdit = true;
			$this->template()->assign(array('aForms' => $aEdit, 'bIsEdit' => $bIsEdit));
		}
		
		
		$this->template()
            ->setTitle(_p($bIsEdit ? 'edit_cancellation_options' : 'add_cancellation_options'))
			...
			->setBreadCrumb(_p($bIsEdit ? 'edit_cancellation_options' : 'add_cancellation_options'), $this->url()->makeUrl('admincp.user.cancellations.add', $bIsEdit ? ['id' => $iId] : []), true)
			;
	}
