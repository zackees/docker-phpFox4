...
class Notification_Service_Notification extends Phpfox_Service
{
        ...
        foreach ($aRows as $aRow) {
            ...
            if (Phpfox::isModule($sModule)) {
                ...
                if (($aCallBack = Phpfox::callback($aRow['type_id'] . '.getNotification', $aRow))) {
                    ...
                    $aCallBack['message'] = $this->cleanNotificationMessage($aCallBack['message']);
        ...
        foreach ($aRows as $aRow) {
            ...
            if ($app !== null && $app->notifications && isset($app->notifications->{$app_key_name})) {
                ...
                if (!empty($aNotification['message'])) {
                    $aNotification['message'] = $this->cleanNotificationMessage($aNotification['message']);
                }
    ...
    public function cleanNotificationMessage($sText)
    {
        $sText = Phpfox::getLib('parse.bbcode')->removeTagText($sText);
        $sText = Phpfox::getService('ban.word')->clean($sText);
        $sText = Phpfox::getLib('parse.output')->cleanScriptTag($sText);
        $sText = preg_replace(['/<(span)([^>]*)>(.*?)<\/span>/mi', '/<(a)([^>]*)>(.*?)<\/a>/mi', '/<(b)([^>]*)>(.*?)<\/b>/mi', '/<(strong)([^>]*)>(.*?)<\/strong>/mi'], '[PHPFOX_START]$1$2[PHPFOX_END]$3[PHPFOX_START]/$1[PHPFOX_END]', $sText);
        $sText = preg_replace(['/</','/>/'], ['&lt;', '&gt;'] ,$sText);
        $sText = str_replace(['[PHPFOX_START]', '[PHPFOX_END]'], ['<', '>'], $sText);
        return $sText;
    }
    /**
