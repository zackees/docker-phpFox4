...
class Api_Service_Gateway_Gateway extends Phpfox_Service 
{
	...
	protected $_notRequiredSetting = ['paypal/merchant_id', 'paypal/paypal_email'];
	...
	public function getUserGateways($iUserId)
	{
        ...
        if (false === ($aGateways = $this->cache()->get($sCacheId))) {
            ...
            $aGateways = [];
            $aNoRequiredSettings = $this->getNotRequiredSetting();
            foreach ($aRows as $iKey => $mValue) {
                $aCache = unserialize($mValue['gateway_detail']);
                $bSkip = false;
                foreach ($aCache as $sSettingKey => $sSettingValue) {
                    if (empty($sSettingValue) && !in_array($mValue['gateway_id'] .'/'. $sSettingKey, $aNoRequiredSettings)) {
                        $bSkip = true;
                    }
                }
	...
	public function get($aGatewayData = array())
	{
        ...
        if (false === ($aGateways = $this->cache()->get($sCacheId))) {
            ...
            if (Phpfox::isAppActive('Core_Activity_Points') && !isset($aGatewayData['no_purchase_with_points']) && Phpfox::getUserParam('activitypoint.can_purchase_with_activity_points')) {
                ...
                if (isset($aSetting[$sCurreny]) && is_numeric($aSetting[$sCurreny])) {
     ...
     * @return array
     */
    public function getNotRequiredSetting()
    {
        return $this->_notRequiredSetting;
    }

    /**
     * @param array $notRequiredSetting
     * @return Api_Service_Gateway_Gateway
     */
    public function setNotRequiredSetting($notRequiredSetting)
    {
        $this->_notRequiredSetting = array_merge($this->_notRequiredSetting, $notRequiredSetting);
