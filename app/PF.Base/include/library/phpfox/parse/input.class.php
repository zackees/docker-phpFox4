...
class Phpfox_Parse_Input
{
    ...
    private function _shorten($sTxt, $iLetters)
    {
        ...
        while (1) {
            ...
            if (mb_strlen($sText) == $iLetters) {
                break;
            } elseif (mb_strlen($sText) > $iLetters) {
                $iHighLetter = $iMid;
            } else {
                $iLowLetter = $iMid;
            }
    ...
    public function prepare($sTxt, $bNoClean = true, $extra = [])
    {
        ...
        if (isset($extra['comment']) && isset($sTagger)) {
            Phpfox::getService('user.process')->mailTaggedInComment([
                'tagger_full_name' => $sTagger,
                'text' => $sTxt,
                'comment_id' => $extra['comment'],
                'feed_privacy' => isset($extra['feed_privacy']) ? $extra['feed_privacy'] : [],
            ]);
        }
     ...
     */
    public function prepareTitle($sModule, $sTitle, $sField, $iUserId, $sTable, $mCondition = null, $bCleanOnly = false, $bCache = true)
    {
        static $aTitle = [];
    ...
    private function _utf8ToUnicode($str, $bForUrl = false)
    {
        ...
        if (defined('PHPFOX_UNICODE_JSON') && PHPFOX_UNICODE_JSON === true) {
            $aUnicode = preg_split('//u', $str, -1, PREG_SPLIT_NO_EMPTY);
            foreach ($aUnicode as $char) {
                $thisValue = ord($char);
                if ($thisValue < 128) {
                    $unicode[] = $thisValue;
                } else {
