...
class Phpfox_Url
{
     ...
     * @return string URL.
     */
    public function getFullUrl($bNoPath = false, $bKeepSegmentValue = false)
    {
        if ($bNoPath) {
            return Phpfox_Request::instance()->get(PHPFOX_GET_METHOD);
        }

        return $this->makeUrl('current', null, false, $bKeepSegmentValue);
    }
    ...
    public function makeUrl($sUrl, $aParams = [], $bFullPath = false, $bKeepSegmentValue = false)
    {
        $sUrl = trim($sUrl, '/');
        ...
        if ($sUrl == 'current') {
            ...
            foreach ($this->_aParams as $sKey => $sValue) {
                if (substr($sKey, 0, 3) == 'req') {
                    $sUrl .= ($bKeepSegmentValue ? urlencode(urldecode($sValue)) : urlencode($sValue)) . '.';
                } else {
                    $aParams[ $sKey ] = $sValue;
                }
            }
    ...
    private function _send($sUrl, $iHeader = null)
    {
        ...
        $aRequests = $_REQUEST;
        $isAjaxCall = !empty($aRequests['core']['ajax']);
        if ($isAjaxCall && isset($aRequests['core']['call']) && $aRequests['core']['call'] == 'core.message') {
            echo '<script>window.location.href = \'' . $sUrl . '\';</script>';
            exit;
        }

        if ($isAjaxCall || (defined('PHPFOX_IS_AJAX_PAGE') && PHPFOX_IS_AJAX_PAGE)) {
    ...
    private function _makeUrl(&$aParts, &$aParams)
    {
        ...
        foreach ($aParts as $iPartKey => $sPart) {
            ...
            $sUrls .= str_replace('.', '', $sPart) . (strpos($sPart, '?') !== false ? '' : '/');
        }
                ...
                if (is_array($sValue)) {
                    foreach ($sValue as $subKey => $subValue) {
                        if (is_array($subValue)) {
                            foreach ($subValue as $subKey2 => $sValue) {
                                $sUrls .= $sKey . '[' . $subKey . ']' . (is_scalar($subKey2) ? (is_bool($subKey2)) ? ($subKey2 ? 'true' : 'false') : '[' . (string)$subKey2 . ']' : '[]') . '=' . $sValue . '&';
                            }
                            continue;
                        }
