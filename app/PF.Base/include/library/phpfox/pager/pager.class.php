...
class Phpfox_Pager
{
	...
	public function set($aParams = array())
	{
        ...
        $this->_aParams['keep_segment_value'] = isset($aParams['keep_segment_value']) ? $aParams['keep_segment_value'] : true;
        ...
        if ($this->_aParams['keep_segment_value']) {
            foreach ($this->_aParams as $key => $value) {
                if (substr($key, 0, 3) == 'req') {
                    $this->_aParams[$key] = urldecode($value);
                }
            }
        }
    ...
    private function _getInfo()
    {
        if (Phpfox::isAdminPanel() || $this->isPopup()) {
            ...
            if (count($this->_aParams)) {
                ...
                {
                    if (in_array($iKey, ['phpfox', Phpfox::getTokenName(), 'page', PHPFOX_GET_METHOD, 'ajax_page_display'])) {
                        continue;
                    }
            ...
            if ($this->_iPage != 1) {
                ...
                $aInfo['firstUrl'] = $oUrl->getFullUrl(false, $this->_aParams['keep_segment_value']);

                $oUrl->setParam($this->_sUrlKey, $this->_iPage-1);
                $aInfo['prevAjaxUrl'] = ($this->_iPage-1);
                $aInfo['prevUrl'] = $oUrl->getFullUrl(false, $this->_aParams['keep_segment_value']);
            ...
            for ($i = $nStart; $i <= $nEnd; $i++) {
                if ($this->_iPage == $i) {
                    ...
                    $aInfo['urls'][$oUrl->getFullUrl(false, $this->_aParams['keep_segment_value'])] = $i;
                }
                else {
                    $oUrl->setParam($this->_sUrlKey, $i);
                    $aInfo['urls'][$oUrl->getFullUrl(false, $this->_aParams['keep_segment_value'])] = $i;
            ...
            $oUrl->setParam($this->_sUrlKey, ($this->_iPage + 1));
            $aInfo['nextAjaxUrlPager'] = $oUrl->getFullUrl(false, $this->_aParams['keep_segment_value']);

            if ($this->_iPagesCount != $this->_iPage) {
                $oUrl->setParam($this->_sUrlKey, ($this->_iPage + 1));
                $aInfo['nextAjaxUrl'] = ($this->_iPage + 1);
                $aInfo['nextUrl'] = $oUrl->getFullUrl(false, $this->_aParams['keep_segment_value']);
                Phpfox_Template::instance()->setHeader('<link rel="next" href="' . $aInfo['nextUrl'] . '" />');

                $oUrl->setParam($this->_sUrlKey, $this->_iPagesCount);
                $aInfo['lastUrl']= $oUrl->getFullUrl(false, $this->_aParams['keep_segment_value']);
            ...
            if ($iTotalPage > 1 && $this->_sPagingMode != 'loadmore') {
                ...
                if ($this->_sPagingMode == 'pagination') {
                    $aPagers[] = [
                        'attr' => 'active',
                        'link' => $oUrl->makeUrl('current',['page'=> $iPage], false, $this->_aParams['keep_segment_value']),
                        'label' => $iPage,
                        'page_number' => $iPage
                    ];
                    ...
                    for ($i = 1; $i <= $iWalk; $i ++) {
                        ...
                        if ($iPrev > 0) {
                            array_unshift($aPagers, [
                                'link' => $oUrl->makeUrl('current',['page'=> $iPrev], false, $this->_aParams['keep_segment_value']),
                                'label' => $iPrev,
                                'page_number' => $iPrev
                            ]);
                        }
                        ...
                        if ($iNext <= $iTotalPage) {
                            $aPagers[] = [
                                'link' => $oUrl->makeUrl('current',['page'=> $iNext], false, $this->_aParams['keep_segment_value']),
                                'label' => $iNext,
                                'page_number' => $iNext
                            ];
                        }
                ...
                if ($this->_sPagingMode == 'pagination' && !empty($this->_aParams['pagination_show_first_last'])) {
                    if ($iPage > 1 || $this->_aParams['paging_show_disabled']) {
                        array_unshift($aPagers, [
                            'link' => $oUrl->makeUrl('current',['page'=> 1], false, $this->_aParams['keep_segment_value']),
                            'label' => $aLabel['first'],
                    ...
                    if ($iPage < $iTotalPage || $this->_aParams['paging_show_disabled']) {
                        $aPagers[] = [
                            'link' => $oUrl->makeUrl('current',['page'=> $iTotalPage], false, $this->_aParams['keep_segment_value']),
                            'label' => $aLabel['last'],
            ...
            Phpfox_Template::instance()->assign([
                ...
                'sNextUrl' => $oUrl->makeUrl('current',['page'=> $iPage + 1], false, $this->_aParams['keep_segment_value']),
                'iNextPage' => $iPage + 1
            ]);
        }
    }	
