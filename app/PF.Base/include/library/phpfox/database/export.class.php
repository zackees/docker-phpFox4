 ...
 * @author			phpFox LLC
 * @package 		Phpfox
 * @version 		$Id: export.class.php 7001 2013-12-19 19:26:25Z Fern $
 */
class Phpfox_Database_Export
{
	...
	{
		$this->_aMap['mysqli'] = $this->_aMap['mysql'];
	}	
				...
				switch ($dbms)
				{
					...
					case 'postgres':
                        ...
                        if (isset($column_data[2]) && $column_data[2] == 'auto_increment')
                        {
                            ...
                            if (strpos($line, "CREATE SEQUENCE {$table_name}_seq") === false)
                            {
                        ...
                        else
                        {
							...
							if (in_array($orig_column_type, ['UINT', 'UINT:', 'USINT', 'BOOL', 'TIMESTAMP']))
							{
	...
	public function getType($dbms, $sType)
    {
        $sType = strtoupper($sType);
        if (strpos($sType, ':') !== false)
        {
            list($orig_column_type, $column_length) = explode(':', $sType);
            if (!is_array($this->_aMap[$dbms][$orig_column_type . ':']))
            {
                $column_type = sprintf($this->_aMap[$dbms][$orig_column_type . ':'], $column_length);
            }
        ...
        {
            $column_type = isset($this->_aMap[$dbms][$sType]) ? $this->_aMap[$dbms][$sType] : $sType;
        }
