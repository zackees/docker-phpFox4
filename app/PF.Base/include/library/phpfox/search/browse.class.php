...
class Phpfox_Search_Browse
{
    ...
    public function execute(\Closure $callback = null)
    {
        ...
        $this->_oBrowse->query();
        $this->_aRows = $this->database()->select(Phpfox::getUserField() . ', ' . (isset($this->_aParams['select']) ? $this->_aParams['select'] : '') . (!empty($this->_aParams['alias_select']) ? $this->_aParams['alias_select'] : ($this->_aParams['alias'] . '.*')))
