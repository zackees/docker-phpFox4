...
class Phpfox_Locale
{
    ...
    public function getLangId()
    {
        ...
        if (Phpfox::isUser()) {
            ...
            $sLanguageId = Phpfox::getLib('session')->get('language_id');
        }
    ...
    public function autoLoadLanguage()
    {
        if (Phpfox::getParam('core.auto_detect_language')) {
            if (isset($_SESSION['lang'])) {
                ...
                $sLanguageId = substr($_SERVER['HTTP_ACCEPT_LANGUAGE'], 0, 2);
            }
        }

        if (empty($sLanguageId)
            || empty($this->_aLanguages[$sLanguageId])) {
            if (empty($sLanguageId = Phpfox::getService('language')->getDefaultLanguage())) {
                $sLanguageId = Phpfox::getParam('core.default_lang_id', 'en');
            }
        }
