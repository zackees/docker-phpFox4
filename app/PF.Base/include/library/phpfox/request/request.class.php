...
class Phpfox_Request
{
     ...
     * @param null $sUserAgent Useragent to send.
     * @param null $aCookies ARRAY of any cookies to pass.
     * @param boolean $bFollow .
     * @param null $sEncoding
     * @param bool $bUseTimeout
     * @return mixed FALSE if failed to connect, STRING if anything was returned from the server.
     */
    public function send($sUrl, $aPost = [], $sMethod = 'POST', $sUserAgent = null, $aCookies = null, $bFollow = false, $sEncoding = null, $bUseTimeout = false)
    {
        ...
        if (extension_loaded('curl') && function_exists('curl_init')) {
            ...
            curl_setopt($hCurl, CURLOPT_HTTPHEADER, [
                'Accept-Language: ' . Phpfox::getLib('locale')->getLangId(),
            ]);
            if ($bUseTimeout) {
                curl_setopt($hCurl, CURLOPT_TIMEOUT, 30);
                curl_setopt($hCurl, CURLOPT_TIMEOUT_MS, 30000);
                curl_setopt($hCurl, CURLOPT_CONNECTTIMEOUT_MS, 30000);
            }
            ...
            if ($sEncoding !== null) {
                curl_setopt($hCurl, CURLOPT_ENCODING, $sEncoding);
            }
            // Run the exec
    ...
    public function getIpInfo($sIp, $sPurpose = 'location')
    {
        if (!filter_var($sIp, FILTER_VALIDATE_IP)) {
            $sIp = $this->getIp();
        }
        $sPurpose    = strtolower(trim($sPurpose));
        $aSupport    = array("country", "country_code", "state", "region", "city", "location", "address");
        $aContinents = array(
            "AF" => "Africa",
            "AN" => "Antarctica",
            "AS" => "Asia",
        ...
        $aOutput = 'US';
        if (filter_var($sIp, FILTER_VALIDATE_IP) && in_array($sPurpose, $aSupport)) {
            $aIpData = @json_decode(file_get_contents("http://www.geoplugin.net/json.gp?ip=" . $sIp));
            if (@strlen(trim($aIpData->geoplugin_countryCode)) == 2) {
                switch ($sPurpose) {
                    case "address":
                        $address = array($aIpData->geoplugin_countryName);
                        if (@strlen($aIpData->geoplugin_regionName) >= 1)
                            $address[] = $aIpData->geoplugin_regionName;
                        if (@strlen($aIpData->geoplugin_city) >= 1)
                            $address[] = $aIpData->geoplugin_city;
                        $aOutput = implode(", ", array_reverse($address));
                        break;
                    case "city":
                        $aOutput = @$aIpData->geoplugin_city;
                        break;
                    case "region":
                    case "state":
                        $aOutput = @$aIpData->geoplugin_regionName;
                        break;
                    case "country":
                        $aOutput = @$aIpData->geoplugin_countryName;
                        break;
                    case "country_code":
                        $aOutput = @$aIpData->geoplugin_countryCode;
                        break;
                    default:
                        $aOutput = [
                            "city"           => @$aIpData->geoplugin_city,
                            "state"          => @$aIpData->geoplugin_regionName,
                            "country"        => @$aIpData->geoplugin_countryName,
                            "country_code"   => @$aIpData->geoplugin_countryCode,
                            "continent"      => @$aContinents[strtoupper($aIpData->geoplugin_continentCode)],
                            "continent_code" => @$aIpData->geoplugin_continentCode
                        ];
                        break;
                }
            }
