...
class Phpfox_File
{
    ...
    public function load($sFormItem, $aSupported = [], $iMaxSize = null, $bCheckSpace = true, $iUserId = null)
    {
        ...
        if (in_array('jpg', $aSupported) && in_array('png', $aSupported) && class_exists('Imagick')) {
            $aSupported = array_merge($aSupported, Phpfox_Image::instance()->getNextGenImgFormats());
        }
    ...
    )
    {
        ...
        $bIsNextGen = false;
        if (Phpfox_Image::instance()->isSupportNextGenImg() && !empty($this->_aFile) && Phpfox_Image::instance()->isNextGenImage($this->_sExt)) {
            try {
                $oNextGenImage = new Imagick($this->_aFile['tmp_name']);
                $oNextGenImage->setImageFormat('jpg');
                $oNextGenImage->setImageCompressionQuality(90);
                $sFormItem = $this->_sDestination . md5($this->_aFile['tmp_name'] . PHPFOX_TIME) . '.jpg';
                $this->_sExt = 'jpg';
                $oNextGenImage->writeImage($sFormItem);
                $bIsNextGen = true;
            } catch (ImagickException $e) {
                return Phpfox_Error::set($e->getMessage());
            }
        }
        ...
        if (($sFormItem != "file") && is_file($sFormItem) && filesize($sFormItem) > 0) {
        ...
        if (Phpfox_Image::instance()->isImageExtension($this->_sExt) && Phpfox_Image::instance()->isImage($sDest)) {
            Phpfox_Image::instance()->fixOrientation($sDest, $bCdn);
        }
     ...
     */
    public function unlink($sSrc, $serverId = null)
    {
        try {
            $return = Phpfox::getLib('cdn')->remove($sSrc, $serverId);
        } catch (Exception $e) {
            $return = false;
        }
