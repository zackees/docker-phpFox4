    ...
    <div class="js_pager_view_more_link">
        ...
        {elseif !empty($aAjaxPaging)}
            <div class="js_pager_buttons" data-block="{$aAjaxPaging.block}" data-content-container="{$aAjaxPaging.container}">
                <a class="ajax-paging" {if !empty($aPager.rel)}rel="{$aPager.rel}"{/if} data-params="{$aAjaxPaging.sParam}&page={$iNextPage}&type=loadmore" href="javascript:void(0);">
                    {_p var='load_more'}
                </a>
            </div>
        {else}
    ...
    <div class="js_pager_buttons" {if !empty($aAjaxPaging)}data-block="{$aAjaxPaging.block}" data-content-container="{$aAjaxPaging.container}"{/if}>
        <ul class="pagination items-{$iTotalPagerItems}">
            ...
            <li class="page-item{if !empty($aPager.attr)} {$aPager.attr}{/if}{if !empty($aPager.rel)} {$aPager.rel}{/if}">
                ...
                {else}
                <a class="page-link{if !empty($aAjaxPaging)} ajax-paging{/if}" {if !empty($aPager.rel)}rel="{$aPager.rel}"{/if} {if !empty($aAjaxPaging)}data-params="{$aAjaxPaging.sParam}&page={$aPager.page_number}" href="javascript:void(0);"{else}href="{$aPager.link}"{/if}>{$aPager.label}</a>
                {/if}
            </li>
            {/foreach}
        </ul>
